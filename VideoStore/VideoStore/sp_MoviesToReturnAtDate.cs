...
public partial class StoredProcedures
{
    ...
    public static void sp_MoviesToReturnAtDate (DateTime DT)
    {
        using (SqlConnection conn = new SqlConnection("context connection=true"))
        {
            ...
            CheckSpecificDate.CommandText = "SELECT CONCAT(Member.FirstName, ' ', Member.LastName) AS Name, MovieInfo.MovieTitle, StartDate, ReturnDate FROM RentOrder INNER JOIN Member ON RentOrder.MemberID = Member.MemberID INNER JOIN MovieInfo ON RentOrder.MovieID = MovieInfo.MovieID WHERE StartDate = DATETIMEFROMPARTS(@year, @month, @day, 0, 0, 0, 0) ORDER BY StartDate";
